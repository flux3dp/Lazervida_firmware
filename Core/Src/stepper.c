...
typedef struct {
  ...
  #endif
  #ifdef VARIABLE_SPINDLE
    uint16_t spindle_pwm; // The TIMx->CCRx value
  #endif
} segment_t;
...
void st_wake_up()
{
  ...
  if (bit_istrue(settings.flags,BITFLAG_INVERT_ST_ENABLE)) { 
    // Motor is active when the disable pin is High
    HAL_GPIO_WritePin(STEP_DISABLE_GPIO_PORT, STEPPERS_DISABLE_PIN, GPIO_PIN_SET);
  } else { 
    // Motor is active when the disable pin is Low
  ...
  __HAL_TIM_SET_PRESCALER(&htim2, st.exec_segment->prescaler);
#endif
  // NOTE: When TIM2->CR1 {URS bit = 0, UDIS = 0},
  //       setting TIM2->EGR UG bit will trigger TIM2 update event 
  //       => trigger the first major action ISR
  HAL_TIM_GenerateEvent(&htim2, TIM_EVENTSOURCE_UPDATE);
  // The next ARR is set in major action  ISR
...
void st_go_idle()
{
  // Disable Stepper Driver Interrupt. (major action ISR)
  // Allow Stepper Port Reset Interrupt (minor ISR) to finish, if active.
...
void st_generate_step_dir_invert_masks()
{
  ...
  #ifdef ENABLE_DUAL_AXIS
    #error "DUAL_STEP_BIT & DUAL_DIRECTION_BIT missing"
...
void st_reset()
{
  ...
  HAL_GPIO_WritePin(STEP_GPIO_PORT, step_port_invert_mask & STEP_MASK, GPIO_PIN_SET);         // Set some step pins
  HAL_GPIO_WritePin(STEP_GPIO_PORT, (~step_port_invert_mask) & STEP_MASK, GPIO_PIN_RESET);    // Reset some step pins
  HAL_GPIO_WritePin(DIR_GPIO_PORT, dir_port_invert_mask & DIRECTION_MASK, GPIO_PIN_SET);      // Set some dir pins
  HAL_GPIO_WritePin(DIR_GPIO_PORT, (~dir_port_invert_mask) & DIRECTION_MASK, GPIO_PIN_RESET); // Reset some dir pins
...
void stepper_init()
{
  ...
  STEP_DDR |= STEP_MASK;
  STEPPERS_DISABLE_DDR |= STEPPERS_DISABLE_PIN;
  ...
  #endif
  */
  MX_TIM2_Init(); // Major action timer
  MX_TIM4_Init(); // Minor timer (for finishing the step)

  // STEP, DIR and STEPPER_DISABLE pins
  GPIO_InitTypeDef GPIO_InitStruct = {0};

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, DIR_Y_Pin|STEP_Y_Pin|DIR_X_Pin|STEP_X_Pin, GPIO_PIN_RESET);
  HAL_GPIO_WritePin(MS1_GPIO_Port, MS1_Pin, GPIO_PIN_SET); // low power mode
  ...
  GPIO_InitStruct.Pin = STEP_EN_INV_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
    ...
    if (pl_block == NULL) {
      ...
      if (prep.recalculate_flag & PREP_FLAG_RECALCULATE) {
        ...
        st_prep_block->direction_bits = pl_block->direction_bits;
            ...
            if (st_prep_block->direction_bits & DIRECTION_PIN[X_AXIS]) { 
          #elif (DUAL_AXIS_SELECT == Y_AXIS)
            if (st_prep_block->direction_bits & DIRECTION_PIN[Y_AXIS]) { 
          #endif
            #error "DUAL_DIRECTION_BIT missing"
...
/**
 * @brief Called frequently when motion is executing
 *        "At most one" step (falling edge) in each major_step_ctrl_ISR
 *        Also used to compute the motion prep buffer for later
 */
void major_step_ctrl_ISR(
  TIM_HandleTypeDef *htim_major, TIM_HandleTypeDef *htim_pulse) {

  //__HAL_TIM_SET_COUNTER(htim_major, 0); 

  if (busy) { return; } // The busy-flag is used to avoid reentering this interrupt

  // Set the direction pins a couple of nanoseconds before we step the steppers
  HAL_GPIO_WritePin(DIR_GPIO_PORT, st.dir_outbits & DIRECTION_MASK, GPIO_PIN_SET);
  HAL_GPIO_WritePin(DIR_GPIO_PORT, (~st.dir_outbits) & DIRECTION_MASK, GPIO_PIN_RESET);
  #ifdef ENABLE_DUAL_AXIS
  ...
  // Then pulse the stepping pins
    ...
    #endif
  #else  // Normal operation
    HAL_GPIO_WritePin(STEP_GPIO_PORT, st.step_outbits & STEP_MASK, GPIO_PIN_SET);
    HAL_GPIO_WritePin(STEP_GPIO_PORT, (~st.step_outbits) & STEP_MASK, GPIO_PIN_RESET); 
    #ifdef ENABLE_DUAL_AXIS
  ...
  __HAL_TIM_SET_AUTORELOAD(htim_pulse, st.step_pulse_time - 1);
  HAL_TIM_GenerateEvent(htim_pulse, TIM_EVENTSOURCE_UPDATE);
  __HAL_TIM_CLEAR_IT(htim_pulse, TIM_IT_UPDATE);
  HAL_TIM_Base_Start_IT(htim_pulse);
  ...
  if (st.exec_segment == NULL) {
    ...
    if (segment_buffer_head != segment_buffer_tail) {
      ...
      // set the frequency of major ISR
      __HAL_TIM_SET_AUTORELOAD(htim_major, st.exec_segment->cycles_per_tick - 1);
  ...
  if (st.counter_x > st.exec_block->step_event_count) {
    st.step_outbits |= STEP_PIN[X_AXIS];
  ...
  if (st.counter_y > st.exec_block->step_event_count) {
    st.step_outbits |= STEP_PIN[Y_AXIS];
    #if defined(ENABLE_DUAL_AXIS) && (DUAL_AXIS_SELECT == Y_AXIS)
      st.step_outbits_dual = (1<<DUAL_STEP_BIT);
    #endif
    st.counter_y -= st.exec_block->step_event_count;
    if (st.exec_block->direction_bits & DIRECTION_PIN[Y_AXIS]) { sys_position[Y_AXIS]--; }
    else { sys_position[Y_AXIS]++; }
  }
#if N_AXIS > 2
  #error "Z_STEP_BIT & Z_DIRECTION_BIT not defined"
  #ifdef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
...
/**
 * @brief Always called to finish a step (rising edge)
 */
void minor_step_ctrl_ISR(TIM_HandleTypeDef *htim_minor) {
  // Reset stepping pins (without change the direction pins)
  /* NOTE: TIM Status bit has been checked and cleared outside the callback */
  HAL_TIM_Base_Stop_IT(htim_minor);  // Stop by its own ISR (enabled by major action ISR)
  HAL_GPIO_WritePin(STEP_GPIO_PORT, step_port_invert_mask & STEP_MASK, GPIO_PIN_SET);      // For inverted step pins
  HAL_GPIO_WritePin(STEP_GPIO_PORT, (~step_port_invert_mask) & STEP_MASK, GPIO_PIN_RESET); // For normal step pins
  #ifdef ENABLE_DUAL_AXIS
